...
def process_round_end_cards(
    ...
    for joker in jokers:
        # ---------------------------------------------------------------
        # calculate_rental — card.lua:2271-2276. Tallied for reporting;
...
def is_rental(card: Card) -> bool:
    """Rental is a sticker, not an ability, and has no debuff gate."""
...
def reset_round_targets(
    ...
    * ``reset_castle_card()`` — picks a random non-Stone card → suit only
    ...
    # ------------------------------------------------------------------
    # The "id" field is what the j_idol handler actually compares against
    # (GameSnapshot.idol_card.get("id") == other_card.get_id()); omitting it
    # made The Idol unable to fire through the real pipeline while its
    # hand-built-ctx unit tests passed — the Throwback bug class.
